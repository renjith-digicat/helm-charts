...
{{/*
Return the proper wasp-ws-reading-service image name
*/}}
{{- define "wasp-ws-reading-service.image" -}}
...
{{- include "common.images.pullSecrets" (dict "images" (list .Values.image .Values.initReadingNotifications.image ) "global" .Values.global) -}}
{{- end -}}

{{/*
Create the name of the service account to use
*/}}
{{- define "wasp-ws-reading-service.serviceAccountName" -}}
...
*/}}
{{- define "wasp-ws-reading-service.kafka.fullname" -}}
...
Usage:
{{ include "wasp-ws-reading-service.kafka.listenerType" ( dict "protocol" .Values.path.to.the.Value ) }}
*/}}
{{- define "wasp-ws-reading-service.kafka.listenerType" -}}
...
{{- $kafkaFullname := include "wasp-ws-reading-service.kafka.fullname" . -}}
...
*/}}
{{- define "wasp-ws-reading-service.kafka.bootstrapBroker" -}}
{{- $releaseNamespace := .Release.Namespace -}}
{{- $clusterDomain := .Values.clusterDomain -}}
{{- $kafkaFullname := include "wasp-ws-reading-service.kafka.fullname" . -}}
{{- $kafkaPort := int .Values.kafka.service.ports.client -}}
...
*/}}
{{- define "wasp-ws-reading-service.initReadingNotifications.image" -}}
{{ include "common.images.image" (dict "imageRoot" .Values.initReadingNotifications.image "global" .Values.global) }}
{{- end -}}

{{/*
Compile all warnings into a single message, and call fail.
*/}}
{{- define "wasp-ws-reading-service.validateValues" -}}
{{- $messages := list -}}
{{- $messages := append $messages (include "wasp-ws-reading-service.validateValues.authentication.sasl" .) -}}
{{- $messages := append $messages (include "wasp-ws-reading-service.validateValues.authentication.tls" .) -}}
...
{{- define "wasp-ws-reading-service.validateValues.authentication.sasl" -}}
{{- $kafkaProtocol := include "wasp-ws-reading-service.kafka.listenerType" ( dict "protocol" (ternary .Values.kafka.auth.clientProtocol .Values.externalKafka.auth.protocol .Values.kafka.enabled) ) -}}
{{- if .Values.kafka.enabled }}
...
wasp-ws-reading-service: kafka.auth.jaas.clientUsers kafka.auth.jaas.clientPasswords
...
wasp-ws-reading-service: externalKafka.auth.jaas.user externalKafka.auth.jaas.password
    It's mandatory to set the SASL credentials when enabling SASL authentication with Kafka brokers.
    You can specify these credentials setting the parameters below:
...
{{- define "wasp-ws-reading-service.validateValues.authentication.tls" -}}
{{- $kafkaProtocol := include "wasp-ws-reading-service.kafka.listenerType" ( dict "protocol" (ternary .Values.kafka.auth.clientProtocol .Values.externalKafka.auth.protocol .Values.kafka.enabled) ) -}}
